...
class RouterTest extends TestCase
{
    ...
    public function testThrowUnAuthorizedMethodExceptionOn()
    {
        ...
        $router = new Router();
        $router->get('/root', 'root', function(){echo 'hello world';});
    }

    public function testGet()
    {
        $route = $this->router->get('/root', 'root', function(){echo 'hello world';});
        self::assertInstanceOf(Route::class,$route);
    }

    public function testPost()
    {
        $route = $this->router->post('/root', 'root', function(){echo 'hello world';});
        self::assertInstanceOf(Route::class,$route);
    }

    public function testGenerateUrl()
    {
        $this->router->get('/simple/:id','simple_show',function(){return true;})
			->with('id','[\d+]');
        $url = $this->router->generateUrl('simple_show', ['id' =>3]);
        self::assertEquals('/simple/3',$url);
        $this->router->get('/simple1/:id/:a_id','simple_show_2',function(){return true;})
